...
public class ChatRoomService {
    ...
    // Mapa que asocia el nombre del usuario a la sala
    ...
    public Set<String> getActiveRoomsAsignadas(ArrayList<Integer> usuariosAsignados) {
        // Crear un conjunto para almacenar las salas filtradas
        Set<String> filteredRooms = new HashSet<>();

        // Recorrer todas las salas activas y agregar las que coincidan con los usuarios asignados
        for (String room : activeRooms) {
            // Verificar que la sala siga el formato "room_<idUsuario>"
            if (room.startsWith("room_")) {
                try {
                    // Extraer el idUsuario después de "room_"
                    Integer roomId = Integer.parseInt(room.substring(5));
                    // Comprobar si el idUsuario está en la lista de usuarios asignados
                    if (usuariosAsignados.contains(roomId)) {
