...
<!-- Script Principal para Manejar el Carrito -->
    ...
    // Lógica especial para recomendados (opcional)
    console.log('Agregado desde recomendados');
} else if (context === 'general') {
    // En general no hacemos nada especial
    console.log('Agregado desde vista general');
}
    // Efecto de éxito:
    // Cambiamos a verde con check por un momento
    button.removeClass('btn-adding');
    // Deja el botón sin texto antes de mostrar el check verde
    button.html('&nbsp;');
    ...
    // No hay más AJAX ni preventDefault aquí.
} else {
...
// Error, restaurar el botón
    ...
    $.ajax({
    url: '/UsuarioFinal/eliminarProductoCarritoAjax/' + idProducto, // Ruta AJAX correcta
    type: 'DELETE',
    success: function (response) {
    if (response.success) {
    ...
    $.ajax({
    url: '/UsuarioFinal/actualizarCantidadCarritoAjax', // Ruta AJAX correcta
    type: 'POST',
    data: {
    ...
    const hiddenInput = button.closest('.mb-4').nextElementSibling.querySelector('.quantity-hidden-field');
    ...
    const hiddenInput = input.closest('.mb-4').nextElementSibling.querySelector('.quantity-hidden-field');

    // Asegurarse de que el valor esté entre 1 y el stock disponible
